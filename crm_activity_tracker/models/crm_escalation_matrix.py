...
class ErpEscalationMatrix(models.Model):
    ...
    managers_ids = fields.One2many('escalation.matrix.hr.employee', 'escalation_matrix_id')
    ...
    def create(self, vals):
        ...
        if vals.get('escalation_matrix_number', _('New')) == _('New'):
            vals['escalation_matrix_number'] = self.env['ir.sequence'].next_by_code('erp.escalation.matrix') or _('New')
...
class ErpEscalationCrm(models.Model):
    ...
    def create(self, vals_list):
        ...
        if vals_list and 'user_id' in vals_list[0]:
            ...
            if len(activity_summary) > 1:
                vals_list[0].update({'next_activity_sequence': activity_summary[1].sequence})
    ...
    def write(self, vals):
        ...
        if 'user_id' in vals and not self.activity_ids and not self.activity_sequence_flag:
            activity_summary = self.env['erp.crm.activity.summary'].search([]).sorted(key=lambda e: e.sequence)
            if len(activity_summary) > 1:
...
class ErpScheduleActivity(models.Model):
    ...
    next_mail_due_date = fields.Date(string='Next mail Remainder Date', compute='compute_next_mail_due_date')
    ...
    def compute_next_mail_due_date(self):
        for rec in self:
            if rec.res_model == 'crm.lead' and rec.date_deadline:
                ...
                rec.next_mail_due_date = rec.date_deadline + relativedelta(days=rec.mail_reminder_day)
    ...
    def action_feedback_schedule_next(self, feedback=False):
        ...
        action = super(ErpScheduleActivity, self).action_feedback_schedule_next(feedback=feedback)
    ...
    def crm_send_email(self):
        crm_mail_template_id = self.env.ref('crm_activity_tracker.mail_crm_schedule_activity')
        if crm_mail_template_id:
            ...
            for rec in activities:
                ...
                escalation_matrix = self.env['erp.escalation.matrix'].search(
                    [('type_of_matrix', '=', crm_obj.type_of_matrix)])

                if escalation_matrix:
                    if not rec.is_mail_triggered:
                        salespersons_ids = escalation_matrix.managers_ids.sorted('serial_no', reverse=True)
                        if salespersons_ids:
                        ...
                        if manager_ids:
                            emails = set(manger_id.work_email for manger_id in manager_ids)
                            values['email_to'] = ','.join(emails)
                            next_sequence_manager_ids = escalation_matrix.managers_ids.filtered(
                                lambda manager_id: manager_id.serial_no < rec.next_mail_manager_sequence)
                            sorted_manager = next_sequence_manager_ids.sorted('serial_no', reverse=True)
                            if sorted_manager:
                                rec.next_mail_manager_sequence = sorted_manager[0].serial_no

                else:
                    _logger.error("No escalation matrix is defined for " + str(crm_obj.type_of_matrix))
...
class ErpActivityType(models.Model):
    ...
    escalation_matrix_id = fields.Many2one('erp.escalation.matrix', string="Escalation Matrix")
...
class ErpCrmTeam(models.Model):
    ...
    activity_id = fields.Many2many('mail.activity.type', string='Activity', domain="[('is_erp_activity', '=', True)]")
